...
class AuthenticationService: ObservableObject {
    ...
    // MARK: - Account Recovery and Security
    
    /// Check for duplicate phone number accounts (for debugging)
    func checkForDuplicatePhoneAccounts() async -> [String] {
        print("🔍 Checking for duplicate phone number accounts...")
        
        do {
            let snapshot = try await firestore.collection("users")
                .whereField("phoneNumber", isNotEqualTo: "")
                .getDocuments()
            
            var phoneNumberCounts: [String: Int] = [:]
            var duplicatePhoneNumbers: [String] = []
            
            for document in snapshot.documents {
                if let phoneNumber = document.data()["phoneNumber"] as? String {
                    phoneNumberCounts[phoneNumber, default: 0] += 1
                    
                    if phoneNumberCounts[phoneNumber] == 2 {
                        duplicatePhoneNumbers.append(phoneNumber)
                        print("🚨 FOUND DUPLICATE: Phone number \(phoneNumber) has multiple accounts")
                    }
                }
            }
            
            print("🔍 Duplicate check complete. Found \(duplicatePhoneNumbers.count) phone numbers with multiple accounts")
            return duplicatePhoneNumbers
            
        } catch {
            print("❌ Error checking for duplicate phone accounts: \(error)")
            return []
        }
    }
    
    /// Clean up duplicate phone number accounts (admin function)
    func cleanupDuplicatePhoneAccounts() async throws {
        print("🧹 Starting cleanup of duplicate phone number accounts...")
        
        let duplicatePhoneNumbers = await checkForDuplicatePhoneAccounts()
        
        for phoneNumber in duplicatePhoneNumbers {
            print("🧹 Cleaning up duplicate accounts for phone: \(phoneNumber)")
            
            let snapshot = try await firestore.collection("users")
                .whereField("phoneNumber", isEqualTo: phoneNumber)
                .getDocuments()
            
            // Sort by creation date to keep the oldest account
            let sortedDocuments = snapshot.documents.sorted { doc1, doc2 in
                let date1 = doc1.get("createdAt") as? Timestamp ?? Timestamp(date: Date.distantPast)
                let date2 = doc2.get("createdAt") as? Timestamp ?? Timestamp(date: Date.distantPast)
            ...
            for i in 1..<sortedDocuments.count {
                ...
                print("🧹 Deleting duplicate account: \(documentToDelete.documentID)")
                try await documentToDelete.reference.delete()
            }
        }
        
        print("🧹 Cleanup complete. Removed \(duplicatePhoneNumbers.count) duplicate accounts")
    }
}
